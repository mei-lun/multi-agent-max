...
标准结果对象是 Executor Adapter 与 Scheduler 之间的控制信封，不是角色聊天正文或所有业务 Artifact 的统一格式。角色可以按 Workflow 声明交付 JSON、Markdown、文件集、代码 diff、测试报告或自然语言报告；MAM 从 workspace、结构化 Executor 事件或明确的最终响应中收集交付物，再构造并校验标准结果。不得对包含进度说明、commentary 或其他会话文本的整段助手消息直接执行 `JSON.parse()`。

`review_gate` 的审核决定必须最终归一化为结构化 `status`、`summary` 和 `findings`，但 Review Role 的最终报告可以是 JSON、Markdown 或含明确结论的自然语言。MAM 在 Artifact Contract 校验和状态推进前完成确定性归一化；存在多个无阶段候选、缺少明确最终响应或无法可靠判断结论时保留 Local Execution Draft，不得猜测、解析终端尾部或自动再次调用模型。
...
- Design Assistant 使用已有 Model Profile，通过可恢复的本地多轮对话进行单问题澄清、二至三个方案比较和至少三个设计部分的结构化建议，同时指出并修复工作流缺陷；方案选择、章节确认和缺陷提示服务于协作但不构成逐步门禁，用户可在认为合适时直接确认完整替换草稿；每条用户消息最多调用模型一次，解析、引用和编译由 MAM 本地静态检查且不自动调用模型修复；不自动读取项目文件/Git 历史，不启动外部可视化服务；实际解析、引用、编译错误或过期基线仍必须阻止创建；最终生成全新角色和工作流，或基于所选现有工作流生成同一 ID 的下一版本；每个可执行节点固定一个角色；确认后只增加定义版本且不产生 Run、Task 或 Attempt，既有 Run 继续固定原版本。
