...
**日期**：2026-08-06
**状态**：Accepted  
**唯一产品基线**：[`docs/final-reuse-integration-plan.md`](../final-reuse-integration-plan.md) 2.2
...
| DEC-003 | 每个可执行 Workflow 节点在设计时固定且只固定一个 Role；Task 创建时继承该角色，用户点击运行时直接使用，不再选角或改派。Claim 提供 Task 级排他执行权；Kernel 拒绝第二个普通领取，用户可以显式强制接管。                                                                                                                                                                                                                                                                                                                                                                                       |
...
| DEC-010 | 采用 Formal Delivery 历史策略：只有通过 Result、Artifact、Git 和 Claim generation 校验的正式交付创建 Attempt 并永久保留；Provider 超时、本地重试和放弃的 Local Execution Draft 不进入 Git Attempt 历史。默认审核 `currentDeliveryAttemptId`，历史 Formal Revision 只读打开，代码使用 Git diff，首期不做 Artifact 专用并排比较。                                                                                                                                                                                                                                                                                   |
...
| DEC-018 | 每个可执行角色节点原生具备任务绑定的人机澄清能力，该能力不是可移除的 Skill。任务明确时自动执行；遇到会实质改变结果且无法从现有上下文查明的问题时，角色必须暂停当前 Task 及其依赖下游并请求人工输入，不得猜测、超时自动回答或自动 fallback；无依赖的并行分支继续执行，跨分支问题可显式申请暂停整个 Run。                                                                                                                                                                                                                                                               |
| DEC-019 | 人工输入统一进入“待我处理”队列。角色可按批次提出最多五个相互独立的问题；决策问题必须提供二至三个有实质差异的方案、取舍、唯一推荐方案和推荐理由，事实问题使用自由文本；用户可逐项或批量选择并提交。角色收到回答后必须复述最终理解；用户可以确认，也可以填写补充意见要求继续澄清，只有确认沟通完成后才能恢复执行。`human_review_gate` 默认把 `changes_requested` 返回产出受审 Artifact 的固定上游角色节点，先完成同样的返工沟通，再由用户确认创建新 Attempt。                                                                                                                        |
...
| `MAM-STATE-002`    | retained/replaced | `MAM2-STATE-002`                         | 保留真实 Git、两 clone、并发写入和冲突；CAS 保护排他 Claim generation 和状态一致性。            |
...
| `MAM-DEVICE-002`   | removed/replaced  | `MAM2-PRESENCE-001`                      | 删除 Device Registry 和 heartbeat；使用 Task 级排他 Claim、generation fencing 与显式强制接管。 |
...
| `MAM-E2E-002`      | replaced          | `MAM2-E2E-002`                           | 两 clone、固定节点角色、排他 Claim、强制接管、过期交付拒绝和 Git 收敛。                        |
...
| `INV2-012` | Claim 不能改变 Workflow 节点固定角色，但它是 Task 级排他执行权；运行时 `reassign_task` 必须被拒绝。         |
...
- Replaced：设备派发和人工选角改为 Workflow 节点固定角色；删除 Device Registry 与 heartbeat，改为 Task 级排他 Claim 和显式强制接管。
...
| `M7-N1..N3` Device/Lease/Recovery                         | replaced          | Workflow 固定角色、Task Claim、Local Execution Draft 与 Formal Delivery。              |
...
| `M10-N2` 两设备排他派发                                   | replaced          | 两 clone、固定节点角色、排他 Claim、强制接管、stale fencing 和 Git 收敛。             |
...
| `MAM2-HUMAN-001`    | 每个固定角色 Task 的 Local Execution Draft 均可通过统一 Application API 进行任务绑定的人工澄清；每批最多五个独立问题，决策问题含二至三个方案、唯一推荐及理由，允许自定义回答。用户确认前不得恢复执行。未交付 Draft 的对话仅保存在本机，Formal Delivery 记录影响结果的简明决策摘要；正式 approval、human review 和分歧决定继续写入 Git 权威事件。默认只暂停当前 Task 与依赖下游；Run 级暂停必须显式声明。                                                                                                                                                                                              |
| `MAM2-HUMAN-002`    | 可视化 Workflow 支持 `human_review_gate`；门禁绑定最新不可变 Attempt/Artifact/commit，`approved` 进入下游，`changes_requested` 要求非空意见并默认返回 Artifact 的固定上游角色节点。返工前必须完成多轮沟通并由用户确认，随后为原 Task 创建带 `previousAttemptId` 的新 Attempt；返工路径必须有界，达到上限进入 blocked。                                                                                                                                                                                                                                                                   |
...
| `MAM2-STATE-002`    | 配置 remote 时两个真实 clone/进程并发提交合法命令并收敛；同一 Task 最多一个 active Claim，CAS retry 在最新 projection 重校验 generation。无 remote 时本地 Git 分支可由多个本地角色复用。加载 unborn 项目时不创建或移动项目 `HEAD`、不提交用户文件，并初始化独立 `mam-state`。用户明确领取首个需要写工作区的 Task 时，完全为空且干净的项目自动创建首个空 commit；存在 staged、modified 或 untracked 文件时返回 `project_initial_commit_required`。                                                                                                                                                                      |
...
| `MAM2-PRESENCE-001` | 两 clone 普通领取同一 Task 时只有一个 active Claim；另一方收到 `task_already_claimed`。用户可以填写原因强制接管并递增 generation；旧领取者的迟到交付或 Review 返回 `stale_claim`。Claim 不依赖 Device Registry、heartbeat 或自动过期。                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `MAM2-RESULT-001`   | 未交付执行只形成本地 Local Execution Draft；标准结果、Artifact、commit、Effective Config 和 Claim generation 全部校验通过后，Scheduler 才在一个批次中创建不可变正式 Attempt 并更新 `currentDeliveryAttemptId`。                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `MAM2-ARTIFACT-001` | 关键 Artifact Git 可读；本地大对象有 hash/availability；Formal Delivery 与历史 Formal Revision 永久可查，本地失败 Draft 不进入 Git Artifact/Attempt 历史。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `MAM2-REVIEW-001`   | Review 绑定 `currentDeliveryAttemptId` 的不可变 Formal Delivery；新 Formal Revision 使旧活动 Review、panel、merge readiness 和下游 lineage 失效。一个 Review Gate 固定一个 Reviewer Role，不同 Reviewer Role 使用多个 Review Gate，并通过 `review_aggregate` 按稳定 slot 和 quorum 确定性汇总。                                                                                                                                                                                                                                                                                                                                                      |
| `MAM2-MERGE-001`    | merge queue 按 `(mergeReadyAt, taskId)` 串行；冲突解决留完整 lineage。`approval_gate` 仅在前置节点通过后可操作。`git_merge.validations` 只接受可执行命令，在 integration worktree 合并后、push 前实际运行，不依赖 Agent 自报同名 verification；历史自然语言 validation 不得阻塞已审核 Run。后续 Role 消费上游 Role 文件 Artifact 时必须等待 Workflow 指定的 `git_merge` 完成，以 Git ancestry 选择唯一终端 `mergeCommit` 创建 Attempt，并接收含来源、路径或 glob、hash 与 commit 的输入清单；未集成、多基线歧义或同一动态节点内的兄弟 Task 文件 Artifact 交接时拒绝启动或创建 Task Plan。 |
| `MAM2-RECOVERY-001` | Provider timeout、Pi 进程重启和应用重启优先恢复同一个 Local Execution Draft、预分配 Attempt ID、Pi session、冻结配置和 worktree，不创建共享 Attempt。未知非幂等副作用进入 `needs_attention` 且不自动继续。用户从头重置或丢弃未交付 Draft 时清理本地执行数据；强制接管后旧 generation 的迟到结果被拒绝。清理并启动同 Workflow 版本、同输入的新 Run 时，仍可验证正式交付 commit 和 Attempt branch 的已提交静态任务可通过完整来源 lineage 复用；已通过且绑定同一 Formal Delivery 的 Review node可复用。                                                                               |
...
| `MAM2-E2E-002`      | 两 clone 使用同一节点固定角色领取同一 Task，不出现角色选择；普通并发领取只有一个成功，显式强制接管后旧 generation 交付被拒绝，Git 状态收敛且本地失败 Draft 不产生共享 Attempt。                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
