...
发生冲突时，以最终设计 2.1 为准。`MAM_COMPLETION_REQUIREMENTS.md` 和其他旧设计均为历史输入，不再是独立完成判据。
...
| DEC-003 | Task 由用户人工分配给 Role；无活动 Attempt 且下一次执行尚未开始时，用户可在 Run 冻结 Role catalog 和 Task allowlist 内修正 Assignment，历史 Attempt 不变。Claim 只提供非排他提示；重复执行由用户避免，Kernel 不拒绝第二个 Attempt。                                |
| DEC-004 | Workflow Run 固定 Workflow 和可分配 Role version catalog，Task 创建时固定自身定义；Role 编辑或新增只影响新 Run。每个 Attempt 解析最新 Executor、Provider、Model、Skill、MCP 和 Knowledge Base，并冻结 Effective Config Snapshot。 |
...
| DEC-016 | Design Assistant 直接使用已有 Model Profile；未加密对话草稿只保存在本机；人工确认只创建全新的 Role Profile 和 Workflow Definition，不启动 Run、不分配 Task。                                                                      |
...
| `MAM-DEVICE-001`   | removed/replaced  | `MAM2-ASSIGN-001`                        | 删除 Device Registry 和设备派发，改为用户人工 Role Assignment。                                |
...
| `MAM-E2E-002`      | replaced          | `MAM2-E2E-002`                           | 两 clone、人工 Assignment、重复执行 warning 和 Git 收敛，不验证 Device/Lease 排他性。          |
...
| `INV2-012` | Claim 只能提示，不能授予或撤销 Assignment，也不能成为排他状态锁；Assignment 修正只能来自带旧值校验的用户命令。 |
...
- Replaced：设备派发改为人工 Role Assignment；lease/heartbeat 改为非排他 execution notice。
...
| `M7-N1..N3` Device/Lease/Recovery                         | replaced          | 人工 Role Assignment、advisory execution warning、两个 Attempt 保留和 Attempt recovery。 |
...
| `M10-N2` 两设备排他派发                                   | replaced          | 两 clone、人工 Assignment、重复 warning、两个 Attempt 和 Git 收敛。                      |
...
| `MAM2-WORKFLOW-001` | 可视化创建节点/边、审核、显式有界循环和 merge，并与 Definition round-trip。                                                                                                                                                                                                                                                                                                                                                            |
...
| `MAM2-DESIGN-001`   | Design Assistant 选择已有 Model Profile 对话生成全新的完整 Role Profile 和一个 Workflow Definition；提供可编译标准模板；模型始终返回完整替换方案，解析、引用和编译错误经有界修复后仍失败时必须保留草稿并允许重试或恢复模板；未加密草稿只保存在本机；人工确认后只创建定义，不创建 Run、Assignment、Task、Attempt 或 Review。                                                                                                            |
...
| `MAM2-ASSIGN-001`   | 只有用户命令产生正式 Role Assignment；Workflow/Agent recommendation 不能替代。无活动 Attempt 且 Task 为 ready/changes_requested 时，用户可通过旧值 CAS 修正为 Run 冻结 catalog 和 Task allowlist 内的其他角色版本；并发改派不能静默覆盖，历史 Attempt 不变。                                                                                                                                                                            |
...
| `MAM2-MERGE-001`    | merge queue 按 `(mergeReadyAt, taskId)` 串行；冲突解决留完整 lineage。                                                                                                                                                                                                                                                                                                                                                                 |
| `MAM2-RECOVERY-001` | Executor 中断后权威 Attempt 不得永久停留在 running；未知非幂等副作用进入 `needs_reconciliation` 且不自动重试。用户确认核对完成后创建唯一 `recovery_planned` Attempt，新的恢复计划或其他并发 Attempt 的成功结果必须封存旧计划，人工启动必须复用其 ID 和 lineage。已恢复 Attempt 的迟到 progress/result，以及 `needs_attention` 期间其他 Executor 的迟到事件，均不得覆盖人工核对状态；历史 Attempt 保留。 |
| `MAM2-OBS-001`      | event/result/log/diagnostics 关联且通过 secret canary；本地日志缺失不破坏 replay。                                                                                                                                                                                                                                                                                                                                                     |
| `MAM2-E2E-001`      | 自定义 Role/Workflow 完成一次代码任务、changes requested、新 Attempt、审核和 merge。                                                                                                                                                                                                                                                                                                                                                   |
| `MAM2-E2E-002`      | 两 clone、人工 Assignment、重复 warning、两个 Attempt 和 Git 收敛。                                                                                                                                                                                                                                                                                                                                                                    |
