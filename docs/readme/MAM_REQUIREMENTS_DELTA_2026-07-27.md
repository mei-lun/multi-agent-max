...
| `MAM2-HUMAN-001`    | 每个固定角色 Task 均可通过统一 Application API 提交任务绑定的人工问题批次并进入 `waiting_for_human_input`；每批最多五个独立问题，决策问题含二至三个方案、唯一推荐及理由，允许自定义回答。用户批量提交后角色可继续追问或给出理解摘要；用户可以确认摘要或填写补充意见要求继续澄清，明确确认前不得恢复执行。默认只暂停当前 Task 与依赖下游；Run 级暂停必须显式声明。所有消息、答案、摘要、补充意见、确认人和时间均写入 Git 事件并可重建。                                                                                                                                                                                         |
