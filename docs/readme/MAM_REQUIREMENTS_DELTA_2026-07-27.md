...
| DEC-002 | Git remote 和 `mam-state` 是跨机器共享状态的唯一权威，不建设中心调度服务。 |
| DEC-003 | Task 由用户人工分配给 Role。Claim 只提供非排他提示；重复执行由用户避免，Kernel 不拒绝第二个 Attempt。 |
...
| `INV2-012` | Claim 只能提示，不能授予或撤销 Assignment，也不能成为排他状态锁。 |
...
- New：MCP tool/resource/prompt 细粒度白名单和 Knowledge Base 只读 Gateway。
...
| `MAM2-UI-001` | Roles、Visual Workflow、Runs、My Role、Task timeline、Reviews、Merge Queue 和 Resources 可操作。 |
| `MAM2-STATE-001` | 删除 projection 后从 Git events 重建相同 hash。 |
| `MAM2-STATE-002` | 两个真实 clone/进程并发提交合法命令并收敛；stale command 在最新 projection 重校验。 |
| `MAM2-STATE-003` | pending conflict 的 resolution batch 被真实应用、持久化并消费。 |
| `MAM2-STATE-004` | 删除或篡改本地 cache 不改变 Git 权威状态。 |
| `MAM2-ASSIGN-001` | 只有用户命令产生正式 Role Assignment；Workflow/Agent recommendation 不能替代。 |
...
| `MAM2-RECOVERY-001` | 崩溃后创建新 Attempt；未知非幂等副作用进入 `needs_reconciliation`。 |
